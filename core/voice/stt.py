...
class STTProvider(Protocol):
    async def transcribe(self, audio_data: bytes, language: str = "", prompt: str = "") -> STTResult:
        ...
...
class DummySTTProvider:
    ...
    async def transcribe(self, audio_data: bytes, language: str = "", prompt: str = "") -> STTResult:
        ...
        return STTResult(text=text, confidence=0.95, duration_ms=100.0, is_final=True)
        ...
        try:
            ...
            with open(tmp.name, "rb") as f:
                transcript = await client.audio.transcriptions.create(
                    file=("audio.wav", f, "audio/wav"), **kwargs,
                )
            text = transcript.text if hasattr(transcript, "text") else transcript.get("text", "")
            raw_segments = transcript.segments if hasattr(transcript, "segments") else transcript.get("segments", [])
            seg_list = []
            avg_logprob = 0.0
            no_speech_prob = 0.0
            if raw_segments:
                for s in raw_segments:
                    sd = dict(s) if hasattr(s, "keys") else {"text": s.get("text", ""), "confidence": s.get("confidence", 1.0), "no_speech_prob": s.get("no_speech_prob", 0.0), "avg_logprob": s.get("avg_logprob", 0.0)}
                    seg_list.append(sd)
            ...
            if no_speech_prob >= self.hallucination_no_speech_threshold:
                return STTResult(text="", confidence=0.0, duration_ms=0.0, is_final=True, no_speech_prob=no_speech_prob, avg_logprob=avg_logprob)
            if confidence < self.hallucination_min_confidence:
                return STTResult(text="", confidence=confidence, duration_ms=0.0, is_final=True, no_speech_prob=no_speech_prob, avg_logprob=avg_logprob)

            lang = transcript.language if hasattr(transcript, "language") else transcript.get("language", language)
            duration = transcript.duration if hasattr(transcript, "duration") else transcript.get("duration", 0)
            return STTResult(
                text=text.strip(), confidence=confidence,
                duration_ms=duration * 1000 if duration else 0, is_final=True,
                language=lang, segments=seg_list,
    ...
    async def transcribe(self, audio_data: bytes, language: str = "", prompt: str = "") -> STTResult:
        ...
        try:
            ...
            segments, info = await asyncio.to_thread(
                ...
                language=language or None, beam_size=5,
            )
            ...
            return STTResult(
                ...
                is_final=True, language=info.language,
                no_speech_prob=no_speech_prob, avg_logprob=avg_logprob,
            )
        finally:
            try:
...
class GoogleSTTProvider:
    ...
    async def transcribe(self, audio_data: bytes, language: str = "", prompt: str = "") -> STTResult:
        ...
        return STTResult(text=text.strip(), confidence=conf, duration_ms=0, is_final=True, language=language or "en-US")
