...
class AudioRecorder:
    ...
    async def record(self, duration: float = 5.0, timeout: float = 10.0) -> bytes:
        ...
        try:
            if self._tool == "arecord":
                ...
                cmd = ["sox", "-q", "-d", "-r", str(self.sample_rate),
                       "-c", str(self.channels), tmp.name,
                       "trim", "0", str(duration)]
    ...
    async def record_until_silence(
        ...
        try:
            ...
            samples = len(raw) // 2
            silence_end = 0
            for i in range(samples - 1, max(0, samples - int(silence_duration * self.sample_rate)), -1):
    ...
    async def calibrate(self, duration: float = 2.0) -> dict:
        ...
        count = len(raw) // 2
        if count == 0:
            return {"ambient_rms": 0, "threshold": 500, "sample_rate": self.sample_rate, "tool": self._tool}
        ...
        return {"ambient_rms": rms, "threshold": threshold, "sample_rate": self.sample_rate, "tool": self._tool}

    @staticmethod
    def _raw_to_wav(raw_data: bytes, sample_rate: int = 16000,
