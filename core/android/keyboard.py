...
class Key:
    ...
    def char_to_keycode(char: str) -> tuple[int, bool]:
        ...
        code_map = {
            ...
            'g': (Key.G, False), 'h': (Key.H, False), 'i': (Key.KEY_I, False),
            'j': (Key.J, False), 'k': (Key.K, False), 'l': (Key.L, False),
            'm': (Key.M, False), 'n': (Key.N, False), 'o': (Key.KEY_O, False),
            ...
            'G': (Key.G, True), 'H': (Key.H, True), 'I': (Key.KEY_I, True),
            'J': (Key.J, True), 'K': (Key.K, True), 'L': (Key.L, True),
            'M': (Key.M, True), 'N': (Key.N, True), 'O': (Key.KEY_O, True),
            ...
            '0': (Key.KEY_0, False), '1': (Key.KEY_1, False),
            '2': (Key.KEY_2, False), '3': (Key.KEY_3, False),
            '4': (Key.KEY_4, False), '5': (Key.KEY_5, False),
            '6': (Key.KEY_6, False), '7': (Key.KEY_7, False),
            '8': (Key.KEY_8, False), '9': (Key.KEY_9, False),
            ' ': (Key.SPACE, False), '.': (Key.PERIOD, False),
            ',': (Key.COMMA, False), '\t': (Key.TAB, False),
            '\n': (Key.ENTER, False),
            '!': (Key.KEY_1, True), '@': (Key.AT, False),
            '#': (Key.POUND, False), '$': (Key.KEY_4, True),
            '%': (Key.KEY_5, True), '^': (Key.KEY_6, True),
            '&': (Key.KEY_7, True), '*': (Key.STAR, False),
            '(': (Key.KEY_9, True), ')': (Key.KEY_0, True),
            '-': (Key.MINUS, False), '_': (Key.MINUS, True),
            '=': (Key.EQUALS, False), '+': (Key.PLUS, False),
            '[': (Key.LEFT_BRACKET, False), '{': (Key.LEFT_BRACKET, True),
            ']': (Key.RIGHT_BRACKET, False), '}': (Key.RIGHT_BRACKET, True),
            '\\': (Key.BACKSLASH, False), '|': (Key.PIPE, False),
            ';': (Key.SEMICOLON, False), ':': (Key.COLON, False),
            "'": (Key.APOSTROPHE, False), '"': (Key.APOSTROPHE, True),
            '/': (Key.SLASH, False), '?': (Key.SLASH, True),
            '`': (Key.GRAVE, False), '~': (Key.TILDE, False),
            '<': (Key.COMMA, True), '>': (Key.PERIOD, True),
        }
...
class LuminaADBKeyboard:
    ...
    def press_dpad(self, direction: str) -> KeyPressResult:
        dir_map = {"up": Key.DPAD_UP, "down": Key.DPAD_DOWN,
                    "left": Key.DPAD_LEFT, "right": Key.DPAD_RIGHT,
                    "center": Key.DPAD_CENTER, "ok": Key.DPAD_CENTER}
    ...
    def press_media(self, action: str) -> KeyPressResult:
        action_map = {"play": Key.MEDIA_PLAY_PAUSE, "pause": Key.MEDIA_PLAY_PAUSE,
                       "stop": Key.MEDIA_STOP, "next": Key.MEDIA_NEXT,
                       "prev": Key.MEDIA_PREVIOUS, "rewind": Key.MEDIA_REWIND,
    ...
    def type_key_by_key(self, text: str, delay_ms: int = 50) -> KeyPressResult:
        ...
        for char in text:
            ...
            try:
                if shift:
                    self.device._adb("shell", "input", "keyevent",
                                     f"--longpress {Key.SHIFT_LEFT} {code}")
                else:
                    self.device.input_keyevent(code)
                count += 1
                if delay_ms > 0:
