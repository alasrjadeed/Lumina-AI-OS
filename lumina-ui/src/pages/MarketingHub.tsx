...
function EmptyState({ icon: Icon, text }: { icon: any; text: string }) { return <div className="text-center py-12 text-[var(--text-tertiary)]"><Icon className="w-12 h-12 mx-auto mb-3 opacity-30" /><p className="text-sm">{text}</p></div>; }
...
export default function MarketingHub() {
  ...
  async function loadAll() {
    setLoading(true);
    try {
      if (tab === 'campaigns') { const r = await get('/marketing/campaigns'); setCampaigns(r.campaigns || []); }
      if (tab === 'content') { const r = await get('/marketing/content'); setContent(r.items || []); }
      if (tab === 'summary') setSummary(await get('/marketing/summary'));
    } catch { /* silent */ }
    setLoading(false);
  }

  async function createCampaign() { try { await post('/marketing/campaigns', cForm); setCForm({ name: '', channel: 'email', budget: 0 }); addToast('Campaign created', 'success'); loadAll(); } catch (e: any) { addToast(e.message, 'error'); } }
  ...
  async function createContent() {
    try { await post('/marketing/content', { ...ctForm, tags: ctForm.tags.split(',').map(t => t.trim()).filter(Boolean) }); setCtForm({ title: '', content: '', channel: 'web', tags: '' }); addToast('Content scheduled', 'success'); loadAll(); } catch (e: any) { addToast(e.message, 'error'); } }
  }
  ...
  function renderCampaigns() {
    if (loading) return <div className="space-y-2">{Array(3).fill(0).map((_, i) => <Skeleton key={i} h="h-20" />)}</div>;
    if (campaigns.length === 0) return <EmptyState icon={Megaphone} text="No campaigns yet" />;
    return (
  ...
  function renderContent() {
    if (loading) return <div className="space-y-2">{Array(3).fill(0).map((_, i) => <Skeleton key={i} h="h-16" />)}</div>;
    if (content.length === 0) return <EmptyState icon={Calendar} text="No scheduled content" />;
    return (
      <div className="space-y-2">
        {content.map((item: any) => (
          <div key={item.title} className="bg-[var(--bg-tertiary)] rounded-xl p-4 flex items-center justify-between hover:bg-[var(--bg-hover)] transition-colors">
            <div>
              ...
              <div className="flex items-center gap-3 mt-1 text-xs text-[var(--text-secondary)]"><span>Channel: {item.channel}</span>{item.tags?.length > 0 && <span>Tags: {item.tags.join(', ')}</span>}</div>
            </div>
  ...
  function renderSummary() {
    if (loading) return <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{Array(4).fill(0).map((_, i) => <Skeleton key={i} h="h-24" />)}</div>;
    if (!summary) return <EmptyState icon={TrendingUp} text="No data available yet" />;
    return (
